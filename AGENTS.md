...
- `/v3/insights` (consistency, sleep debt, breathing disturbances, day/week/month trends) is documented and fetched by the coordinator for both the primary and partner accounts (`data["insights_v3"]` / `data["partner_insights_v3"]`, added 2026-07-16). Consistency now has a sensor (`OrionConsistencySensor` in `sensor.py`, added 2026-07-16 — reads `metrics.consistency` from the day granularity's latest period via `coordinator.get_consistency_metric()` / `get_partner_consistency_metric()`); sleep debt and breathing disturbances still have **no entities/sensors yet**. See follow-up beads filed 2026-07-16 for the remaining scoped sensor implementation work (each includes a partner-side variant).
