...
- `/v3/insights` (consistency, sleep debt, breathing disturbances, day/week/month trends) is documented and fetched by the coordinator for both the primary and partner accounts (`data["insights_v3"]` / `data["partner_insights_v3"]`, added 2026-07-16). Consistency (`OrionConsistencySensor`), breathing disturbances (`OrionBreathingDisturbancesSensor`), and sleep debt (`OrionSleepDebtSensor`) all have dedicated sensors now (each with a partner variant).
