...
- `/v3/insights` (consistency, sleep debt, breathing disturbances, day/week/month trends) is documented and fetched by the coordinator for both the primary and partner accounts (`data["insights_v3"]` / `data["partner_insights_v3"]`, added 2026-07-16). Consistency (`OrionConsistencySensor`, reads `metrics.consistency` from the day granularity's latest period via `coordinator.get_consistency_metric()` / `get_partner_consistency_metric()`), breathing disturbances (`OrionBreathingDisturbancesSensor`), and sleep debt (`OrionSleepDebtSensor`, via `coordinator.get_sleep_debt()` / `get_partner_sleep_debt()`) now all have sensors in `sensor.py`, each with a primary and partner-side variant. Day/week/month trend granularities beyond the latest-day period are still not surfaced as separate entities — only exposed as `vs_prior_day`/`vs_prior_week`/`vs_prior_month` comparison attributes on the relevant sensor.
